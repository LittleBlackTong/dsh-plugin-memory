...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 只读记忆图谱：页间链接 + 共享 tag，力导向布局，悬停高亮邻居。 */
    function GraphCard() {
      ...
      const svg = React.createElement('svg', {
        ...
        width: '100%',
        style: { display: 'block', borderRadius: '8px', background: 'var(--dsw-alias-bg-layer-1, transparent)' },
        role: 'img',
        'aria-label': '记忆图谱',
      },
        edges.map((e, index) => {
          ...
          return React.createElement('line', {
            ...
            stroke: e.kind === 'link' ? 'var(--dsw-alias-brand-primary, #4c8bf5)' : 'var(--dsw-alias-border-l2, rgba(128,128,128,0.45))',
            strokeWidth: e.kind === 'link' ? 1.4 : 0.8,
            strokeOpacity: litEdge(e) ? 0.85 : 0.12,
          })
        }),
        nodes.map((node) => React.createElement('circle', {
            key: node.id,
            ...
            `${node.label}\n${node.type}${node.salience ? ` · salience:${node.salience}` : ''}`
            + `${node.tags && node.tags.length > 0 ? `\n#${node.tags.join(' #')}` : ''}`
            + `\n${node.degree ?? 0} 条关系`,
          )),
        ),
        nodes.map((node) => {
          // Labels only for hubs and well-connected pages: drawing all 33 would
          // overlap into noise, and the rest are one hover away.
          if (!node.hub && (node.degree ?? 0) < 2) return null
      ...
      return React.createElement('div', { style: cardStyle },
        ...
        React.createElement('div', { style: cardSubStyle },
          `${stats.pages ?? nodes.length} 页 · ${stats.links ?? 0} 条页间链接 · ${stats.tagEdges ?? 0} 条共享 tag 关系`
          + `${stats.isolated && stats.isolated.length > 0 ? ` · ${stats.isolated.length} 页暂时孤立` : ''}`,
        ),
        svg,
        React.createElement('div', { style: { marginTop: '8px' } }, ...chips),
        ((stats.isolated && stats.isolated.length > 0) || (stats.links ?? 0) === 0)
          ? React.createElement('div', { style: { fontSize: '11px', lineHeight: '17px', color: 'var(--dsw-alias-label-secondary, #888)', marginTop: '6px' } },
            (stats.links ?? 0) === 0
              ...
              : (stats.isolated && stats.isolated.length > 0)
                ? `这 ${stats.isolated.length} 页还没和别的记忆连上：${stats.isolated.slice(0, 3).map((id) => id.replace(/\.md$/, '')).join('、')}${stats.isolated.length > 3 ? ' 等' : ''}`
                : null,
          )
          : null,
        React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #888)', marginTop: '6px' } },
          '悬停任一节点可高亮它的关系；数据来自页面的 markdown 链接与 frontmatter tags，只读、实时。',
        ),
      )
    }
