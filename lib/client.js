...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    const GraphView = React.memo(function GraphView({ graph }) {
      ...
      // Bloom in from the centre on first paint, unless reduced motion is asked.
      React.useEffect(() => {
      ...
      /** Pointer position in graph coordinates (the viewBox is fixed). */
      ...
      return React.createElement('div', { style: cardStyle },
        ...
        React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #888)', marginTop: '6px' } },
          '可拖动节点整理视图；悬停高亮它的关系。数据来自 markdown 链接与 frontmatter tags，只读、实时。',
        ),
      )
    })

    /** 记忆图谱卡片：负责取数与生命周期，渲染交给 GraphView。 */
    function GraphCard() {
      ...
      return React.createElement(GraphView, { graph })
    }
