...
/** Recent-activity headlines carried in the boot block. Three is enough to
 * orient a session; five cost ~90 characters more per request for no extra
 * signal, and this block is injected on every request. */
export const RECENT_LOG_COUNT = 3
...
/**
 * Read the **last** `maxChars` characters of a file.
 *
 * `readCapped` takes the head, which is right for schema files and wrong for
 * append-only logs: on a 100 KB `log.md` the head is the oldest entries, so the
 * "recent activity" section was quietly showing the *first* five entries the
 * store ever had. Anything that reads a growing timeline from the end needs
 * this instead.
 *
 * @param {string} path
 ...
 */
export function readTailCapped(path, maxChars) {
  if (!existsSync(path)) return undefined
  try {
    const text = readFileSync(path, 'utf8')
    if (text.length <= maxChars) return text
    // Slice by characters, not bytes: a Chinese entry is ~3 bytes per character.
...
export function renderBootBlock(memoryDir, options = {}) {
  ...
  // Read the tail: log.md is append-only, so the head is ancient history.
  const logPath = join(memoryDir, 'log.md')
  const logEntries = existsSync(logPath)
    ? parseLogEntries(readTailCapped(logPath, 4096) ?? '', RECENT_LOG_COUNT)
    : []
