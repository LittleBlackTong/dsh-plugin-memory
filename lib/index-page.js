...
export function clampLabel(title, max = INDEX_LABEL_MAX) {
  ...
  // A label is a road sign, so cut it somewhere meaningful rather than mid-word.
  // CJK titles usually carry a parenthetical or a colon separator that marks a
  // natural short form: "决策：社区上架主线选 awesome-dsh-plugin（喂给 dsh-market）"
  // is better served by "决策：社区上架主线选 awesome-dsh-plugin" than by a
  ...
  if (best > Math.floor(max / 3)) return `${head.slice(0, best).trimEnd()}…`

  // Otherwise prefer a word boundary for latin text ("dsh-plugin-superpowers
  // 插件（obra…" → "dsh-plugin-superpowers…").
  const boundary = head.search(/\s(?=\S*$)/)
  if (boundary > Math.floor(max / 3)) return `${head.slice(0, boundary).trimEnd()}…`
