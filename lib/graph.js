 ...
 */
const WEAK_TAGS = new Set(['skill', 'skills', 'project', 'projects', 'decision', 'decisions', 'concept', 'concepts', 'fact'])
...
export function suggestLinks({ nodes, edges }, options = {}) {
  ...
  for (const page of lonely) {
    ...
    for (const other of pages) {
      ...
      const weakOnly = mine.size === 0 && (page.weakTags?.size ?? 0) > 0
      const unlabelled = mine.size === 0 && (page.tags?.length ?? 0) === 0
      const score = content + weakTags.length + (sameType ? 1 : 0)
      // Title overlap is a real signal regardless of tags, so it counts as
      // content; only the "we are both skills" case is demoted.
      const hasSignal = sharedTags.length > 0 || sharedWords.length > 0
      const qualifies = hasSignal
        ? score >= minScore
        : unlabelled
          ? score >= minScore - 2
          : false
      if (qualifies) {
        scored.push({ other, score, sharedTags: [...sharedTags, ...weakTags], sameType, sharedWords })
      }
    }
  ...
  return out.filter((entry) => entry.candidates.length > 0)
}
