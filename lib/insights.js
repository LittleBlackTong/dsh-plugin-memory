...
export function collectInsights(memoryDir) {
  ...
  // Characters, not bytes: every budget in this plugin is expressed in
  // characters (bootMaxChars), and a CJK page is ~3 bytes per character, so
  // comparing bytes against a character budget overstates by ~2x.
  let indexChars = 0
  if (existsSync(indexPath)) {
    try {
      indexChars = readFileSync(indexPath, 'utf8').length
    } catch {
      indexChars = indexBytes
    }
  }
  const logSize = (() => {
    try {
